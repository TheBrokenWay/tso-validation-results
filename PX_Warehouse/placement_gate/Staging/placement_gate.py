...
def audit_placement(
    ...
    *,
    exclude_dirs: Tuple[str, ...] = (
        "__pycache__", ".git", "Operations", "WorldLines",
        "placement_gate",          # warehouse infrastructure code
        "Archive_Novel", "Archive_Primary", "Backup_Pre_Refinery",  # legacy archives
        "CommercialAssets", "TrialSimulations",  # legacy data dirs
        "PX_LOGS", "PX_Warehouse",              # nested/legacy dirs
        "00_COMMERCIAL_DOSSIERS",               # legacy commercial dossiers
    ),
    ...
    # Pre-resolve zone roots once (not per-file) for fast string comparison
    zone_resolved: List[Path] = [z.resolve() for z in _canonical_zone_roots(repo_root)]
    wh_resolved = wh.resolve()

    # Zone name prefixes for fast counting (relative to wh)
    _zone_prefixes = (
        "Feeder", "Calibration_Molecules", "Learning_Material",
        "Prv_Dossiers", "Novel_Dossiers", "Finalized_Dossiers",
    )
    ...
    def _fast_is_sanctioned(resolved_item: Path) -> bool:
        rstr = str(resolved_item)
        for zr in zone_resolved:
            zstr = str(zr)
            if rstr == zstr or rstr.startswith(zstr + "/") or rstr.startswith(zstr + "\\"):
                return True
        return False

    def _fast_zone_name(rel: Path) -> str | None:
        top = rel.parts[0] if rel.parts else ""
        if top == "Finalized_Dossiers" and len(rel.parts) >= 2 and rel.parts[1] == "Discovery_Accepted":
            return "Discovery_Accepted"
        if top in _zone_prefixes:
            return top
        return None
    ...
    for item in wh.rglob("*"):
        ...
        if not _fast_is_sanctioned(item_resolved):
            misplaced.append(item)
        else:
            zone = _fast_zone_name(rel)
            if zone:
                counts[zone] = counts.get(zone, 0) + 1
            if len(sanctioned_sample) < 50:
